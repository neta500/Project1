...
Acceptor::Acceptor(IoContext& ioContext, const EndPoint& endPoint, const ServerService* service)
	: mIoContext(ioContext), mEndPoint(endPoint), mServerService(service)
{
...
void Acceptor::Accept()
{
	...
	// TODO : AcceptIocpOperation Àç»ç¿ë
	...
	if (false == ::AcceptEx(mListenSocket, session->GetSocket(), session->mRecvBuffer.data(), 0, sizeof(SOCKADDR_IN) + 16, sizeof(SOCKADDR_IN) + 16, &bytesTransferred, overlapped.get()))
	{
		const int error = ::WSAGetLastError();
